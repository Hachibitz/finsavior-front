...
export class MainComponent implements OnInit, AfterViewInit {
  ...
  tableTypes: string[] = ["main", "credit-card"];
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe((params) => {
      const isRedirectToAiAnalysis = String(
        params.get("isRedirectToAiAnalysis")
      );
  ...
  constructor(
    ...
    this.mainTableForm = this.fb.group({
      ...
      billValue: [
        "",
        [Validators.required, numberAndDecimalValidator, Validators.min(1)],
      ],
      billDescription: [""],
  ...
  isMainTableFormValid(): boolean {
    ...
    if (selectedTypeControl.invalid) {
      //(selectedTypeControl.dirty || selectedTypeControl.touched)
  ...
  openAiAdviceDialog(): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === "cancel") {
        ...
        haveCoverage = this.validateSelectedAnalisysAndPlan(
          this.userData,
          result.analysisTypeId
        );
        if (!haveCoverage) {
          this.dialogMessage.openWarnDialog(
            "Seu plano não possui a análise selecionada. Faça o upgrade agora mesmo com condições especiais!"
          );
        }
  ...
  validateSelectedAnalisysAndPlan(
    userData: UserData,
    selectedAnalisys: number
  ): boolean {
    const plan: Plan = PlanEnum.find(
      (profilePlan) => profilePlan.planId == userData.plan.planId
    );
    const coverageList: AnalysisType[] = Object.values(PlanCoverageEnum).find(
      (planCoverage) => planCoverage.planId === plan.planId
    ).coverages;

    const chosenAnalysis: AnalysisType = coverageList.find(
  ...
  addRegisterMain(event: Event) {
    ...
    this.isLoading();
    this.billService
      .billRegister(billRegisterRequest)
      .then((result) => {
        this.setTableData();
      ...
      .catch((error) => {
        this.dialogMessage.openErrorDialog(
          "Falha ao inserir registro, tente novamente mais tarde."
        );
        this.isLoading();
      });
  }
    ...
    if (tableData.mainTableDataList) {
      tableData.mainTableDataList.forEach((row) => {
        if (
          row.billType === "Caixa" ||
          row.billType === "Ativo" ||
          row.billType === "Poupança"
        ) {
          this.incomeRows.data.push({
            id: row.id,
            Nome: row.billName,
            Valor: "R$ " + row.billValue,
            Tipo: row.billType,
            Descricao: row.billDescription,
            Data: row.billDate,
          });
        } else {
          this.rows.data.push({
  ...
  async loadMainTableData(): Promise<void> {
    ...
    try {
      const result = await this.billService.loadMainTableData(
        this.formatData(this.billDate)
      );
  ...
  async loadCardTableData(): Promise<void> {
    ...
    try {
      const result = await this.billService.loadCardTableData(
        this.formatData(this.billDate)
      );
  ...
  syncCardAndMainTableExpenses(): void {
    ...
    if (isCardBillPresent == 0) {
      this.rows.data.push({
        ...
        Valor: "R$ " + creditCardTableAmount.toFixed(2),
        Tipo: "Passivo",
        Descricao: "Soma da tabela de cartão de crédito",
        Data: this.formatData(this.billDate),
        Pago: localStorage.getItem("isCreditCardPaid") == "true",
      });
    } else {
  ...
  setStatusData(): void {
    this.totalPaid = this.rows.data
      .filter((row) => row.Pago)
      .reduce((acc, row) => acc + parseFloat(row.Valor.replace("R$ ", "")), 0);
    this.currentlyAvailableIncome = this.incomeRows.data
      .filter(
        (incomeRow) => incomeRow.Tipo === "Caixa" || incomeRow.Tipo === "Ativo"
      )
      .reduce(
        (acc, incomeRow) =>
          acc + parseFloat(incomeRow.Valor.replace("R$ ", "")),
        0
      );
    this.setTotals();
  ...
  setTotals(): void {
    this.totalDebit = this.rows.data
      .filter((row) => row.Tipo === "Passivo")
      .reduce((acc, row) => acc + parseFloat(row.Valor.replace("R$ ", "")), 0)
      .toFixed(2);
  ...
  editItemFromMainTable(item) {
    ...
    this.billService
      .editItemFromMainTable(billUpdate)
      .then((result) => {
  ...
  editItemFromCardTable(item) {
    ...
    this.billService
      .editItemFromCardTable(billUpdate)
      .then((result) => {
        this.dialogMessage.openInfoDialog(result.message);
        this.refreshTablesAndStatus(this.cardRows);
        this.dialogMessage.openInfoDialog("Item salvo");
  ...
  markAsPaid(item) {
    item.Nome != "Cartão de crédito"
      ? this.editItemFromMainTable(item)
      : localStorage.setItem("isCreditCardPaid", item.Pago);
    this.setTotals();
  }
  ...
  async generateAiAdvice(
    ...
    const [rowsTableString, cardRowsTableString, incomeRowsTableString] =
      await Promise.all([
        this.getMainTableFromAnalysisTypeAndStartingDate(
          analysisTypeId,
          startingDate
        ),
        this.getCardTableFromAnalysisTypeAndStartingDate(
          analysisTypeId,
          startingDate
        ),
        this.getIncomeTableFromAnalysisTypeAndStartingDate(
          analysisTypeId,
          startingDate
        ),
      ]);
  ...
  generateAiAdviceCall(aiAdviceRequest: AiAdviceRequest): void {
    this.isLoading();
    this.billService
      .generateAiAdvice(aiAdviceRequest)
      .then((result) => {
  ...
  async getMainTableFromAnalysisTypeAndStartingDate(
    analysisTypeId: number,
    startingDate: Date
  ): Promise<string> {
    const analysisTypeChosen = this.analysisTypes.find(
      (type) => type.analysisTypeId === analysisTypeId
    );
  ...
  async getIncomeTableFromAnalysisTypeAndStartingDate(
    analysisTypeId: number,
    startingDate: Date
  ): Promise<string> {
    const analysisTypeChosen = this.analysisTypes.find(
    ...
    for (let i = 0; i < analysisTypeChosen.period; i++) {
      ...
      stringBuilder
        .append("\n\n\n\nDados dessa tabela são referentes ao mês e ano: ")
        .append(
          `${this.billDate.getMonth()}/${this.billDate.getFullYear()} \n\n`
        )
  ...
  async getCardTableFromAnalysisTypeAndStartingDate(
    analysisTypeId: number,
    startingDate: Date
  ): Promise<string> {
    const analysisTypeChosen = this.analysisTypes.find(
  ...
  getIncomeTotal(): number {
    let result: number = this.incomeRows.data.reduce(
      (acc, incomeRow) => acc + parseFloat(incomeRow.Valor.replace("R$ ", "")),
      0
    );
    return result;
  }
